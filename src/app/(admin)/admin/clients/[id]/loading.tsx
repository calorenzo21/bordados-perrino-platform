    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-24">
        {Array.from({ length: 4 }).map((_, i) => (
          <div
            key={i}
            className={`h-14 animate-pulse rounded-xl bg-slate-200 ${i === 0 ? 'sm:col-span-6' : i === 3 ? 'sm:col-span-11' : 'sm:col-span-3'}`}
          />
        ))}
      </div>
        ...
        <CardContent className="p-0">
          ...
          <div className="space-y-px">
            {Array.from({ length: 5 }).map((_, i) => (
              <div key={i} className="h-16 mx-6 animate-pulse rounded-xl bg-slate-100" />
            ))}
          </div>
        </CardContent>
      </Card>
