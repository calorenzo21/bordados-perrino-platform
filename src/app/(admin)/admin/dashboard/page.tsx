...
export default function AdminDashboardPage() {
  ...
  // Actualizar el timestamp cuando los datos cambian (cuando isLoading pasa de true a false)
  useEffect(() => {
    if (!isLoading) {
  ...
  const ordersByMonth = chartPeriod === 6
    ? allOrdersByMonth.slice(-6)
    : allOrdersByMonth;
  ...
  const bestMonth = ordersByMonth.length > 0
    ? ordersByMonth.reduce((best, current) =>
      current.orders > best.orders ? current : best
      , ordersByMonth[0])
    : { month: '-', monthFull: '-', orders: 0, revenue: 0 };

  // Calcular promedio mensual
  const averageMonthly = ordersByMonth.length > 0
    ? Math.round(totalOrdersInPeriod / ordersByMonth.length)
    : 0;
  ...
  const previousPeriodOrders = chartPeriod === 6
          ...
          <CardContent>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <div className="group relative overflow-hidden rounded-xl border border-purple-100 bg-gradient-to-br from-purple-50 to-white p-5 transition-all hover:border-purple-200 hover:shadow-md">
                ...
                <div className="relative">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium text-purple-600">Parcialmente Entregado</span>
                  ...
                  <div className="mt-4 flex items-center gap-2">
                    ...
                    <span className="text-xs font-medium text-slate-500">
                      {safePercentage(ordersByStatus[OrderStatus.PARCIALMENTE_ENTREGADO], totalActiveOrders)}%
                    </span>
                  </div>
                </div>
              </div>
            ...
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <div>
                ...
                <div className="mt-2 flex items-baseline gap-2">
                  ...
                  <span className={`inline-flex items-center rounded-md px-1.5 py-0.5 text-xs font-semibold ${parseFloat(percentageChange) >= 0
                    ? 'bg-emerald-50 text-emerald-600'
                    : 'bg-rose-50 text-rose-600'
                    }`}>
                    {parseFloat(percentageChange) >= 0 ? '↑' : '↓'} {Math.abs(parseFloat(percentageChange))}%
                  </span>
                </div>
              </div>
              <div className="flex gap-2">
                <Button
                  ...
                  size="sm"
                  className={`h-8 rounded-lg text-xs ${chartPeriod === 6
                ...
                <Button
                  ...
                  size="sm"
                  className={`h-8 rounded-lg text-xs ${chartPeriod === 12
                    ? 'bg-blue-500 hover:bg-blue-600'
                    : 'border-slate-200'
                    }`}
              ...
              >
                ...
                <div className="text-right">
                  <p className="font-semibold text-emerald-600">
                    ${client.revenue.toLocaleString('es-AR', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
                  </p>
                </div>
              </div>
            ))}
