...
function getDaysIndicator(order: { status: string; isDelayed: boolean; daysRemaining: number }): {
  ...
  if (order.status === OrderStatus.CANCELADO) {
    return { text: 'Cancelado', color: 'text-slate-400 dark:text-slate-500' };
  }
  if (order.isDelayed) {
  ...
  return { text: `${order.daysRemaining}d restantes`, color: 'text-slate-500 dark:text-slate-400' };
}
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-row gap-2">
        <div className="relative flex-1">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400 dark:text-slate-500 z-10 pointer-events-none" />
          <Input
            type="text"
            placeholder="Buscar pedido..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="text-sm h-10 pl-10 rounded-xl border-slate-200 bg-white/80 backdrop-blur-sm focus:border-blue-300 focus:ring-blue-200 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100 dark:placeholder:text-slate-400"
          />
        </div>
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          <SelectTrigger className="w-11 h-10 rounded-xl border-slate-200 bg-white/80 backdrop-blur-sm px-0 justify-center [&>svg:last-child]:hidden dark:border-slate-600 dark:bg-slate-700">
            {(() => {
              ...
              return (
                <div className="relative">
                  <Icon
                    className={`h-4 w-4 ${isFiltered ? selectedOption?.textColor : 'text-slate-500 dark:text-slate-400'}`}
                  />
                  {isFiltered && (
                    <span
          ...
          filteredAndSortedOrders.map((order) => {
            ...
            return (
              <Link key={order.id} href={`/client/orders/${order.orderNumber}`} className="block">
                <div
                  className={`group relative rounded-2xl bg-white/80 backdrop-blur-sm border transition-all duration-200 hover:shadow-lg hover:border-blue-200/50 active:scale-[0.98] active:shadow-md dark:bg-slate-800 dark:hover:border-blue-800/50 ${
                    isCancelled
                      ? 'border-slate-200/30 opacity-60 dark:border-slate-700/30'
                      : isCompleted
                  ...
                  <div className="p-5">
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        <p
                          className={`mt-2 text-sm ${isCancelled ? 'text-slate-400 dark:text-slate-500' : 'text-slate-600 dark:text-slate-300'} line-clamp-1`}
                        >
                          {order.description}
                        </p>
                      </div>
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        ...
                        <ChevronRight className="h-5 w-5 text-slate-300 dark:text-slate-600 transition-transform group-hover:translate-x-1 group-hover:text-blue-500" />
                      </div>
                    </div>
  ...
  return (
    <div className="space-y-5 animate-pulse">
      ...
      <div className="flex flex-row gap-2">
        ...
        <div className="h-10 w-11 rounded-xl bg-slate-200 dark:bg-slate-700/40" />
      </div>
      ...
      <div className="space-y-3">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="rounded-2xl bg-white/80 border border-slate-200/50 p-5 dark:bg-slate-800 dark:border-slate-700"
          >
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 space-y-2">
                <div className="flex items-center gap-2">
                ...
                <div className="h-4 w-48 rounded bg-slate-200 dark:bg-slate-700/60" />
              </div>
              <div className="text-right space-y-1">
            ...
            <div className="mt-4 space-y-2">
              ...
              <div className="h-2.5 w-full rounded-full bg-slate-200 dark:bg-slate-700/40" />
            </div>
          </div>
        ))}
      </div>
